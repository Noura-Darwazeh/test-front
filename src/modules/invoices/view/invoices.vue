...
const invoiceColumns = computed(() => [
  ...
  { key: "invoice_code", label: t("invoice.invoiceCode"), sortable: true },
  {
    ...
    label: t("invoice.deliveryCompany"),
    sortable: false,
  },
  {
    key: "client_company_name",
    label: t("invoice.clientCompany"),
    sortable: false,
  },
  {
    key: "collection_amount",
    label: t("invoice.collectionAmount"),
    sortable: true,
  },
...
const filteredColumns = computed(() => {
  return invoiceColumns.value.filter((col) =>
...
const handleRestore = async (invoice) => {
  try {
    ...
    showError(error.message || "Failed to restore invoice. Please try again.");
  }
};
...
const handlePermanentDelete = async (invoice) => {
  ...
  try {
    ...
    showError(error.message || "Failed to delete invoice. Please try again.");
  }
};
...
const markInvoicesAsPaid = async () => {
  if (selectedRows.value.length === 0) {
    showError(
      t("invoice.selectInvoices") || "Please select at least one invoice",
    );
    return;
  }
  ...
  if (
    !confirm(
      t("invoice.markAsPaidConfirm", { count: selectedRows.value.length }) ||
  ...
  try {
    ...
    if (response.data?.success) {
      await invoicesStore.fetchInvoices();
      selectedRows.value = [];
      showSuccess(response.data.message || t("invoice.markedAsPaidSuccess"));
    }
  } catch (error) {
    console.error("❌ Failed to mark invoices as paid:", error);
    showError(
      error.message ||
        t("invoice.markedAsPaidError") ||
...
const exportSelectedInvoice = async () => {
  if (selectedRows.value.length !== 1) {
    showError(
      t("invoice.selectOneInvoice") || "Please select exactly one invoice",
    );
    return;
  }
...
const handleTrashedBulkAction = async ({ actionId, selectedIds }) => {
  ...
  try {
    if (actionId === "restore") {
      ...
      if (
        !confirm(
          t("common.bulkPermanentDeleteConfirm", {
    ...
    showError(error.message || "Failed to process invoices. Please try again.");
  } finally {
    bulkActionLoading.value = false;
  }
};
...
const exportInvoicePDF = async (invoice) => {
  ...
  try {
    ...
    if (
      deliveryLogo &&
      deliveryLogo.trim() !== "" &&
      deliveryLogo !== "data:image/jpg;base64,"
    ) {
    ...
    if (
      clientLogo &&
      clientLogo.trim() !== "" &&
      clientLogo !== "data:image/jpg;base64,"
    ) {
    ...
    showError(t("invoice.exportError") || "Failed to export PDF");
  } finally {
