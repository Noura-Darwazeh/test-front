<template>
  <div class="collections-page-container bg-light">
    ...
    <ConfirmationModal
      ...
      :message="
        t('collection.createInvoiceConfirm', { count: selectedRows.length })
      "
...
const detailsFields = computed(() => [
  ...
  {
    key: "invoice_code",
    label: t("collection.invoiceCode"),
    colClass: "col-md-6",
  },
  {
    key: "driver_name",
    label: t("collection.driverName"),
    colClass: "col-md-6",
  },
...
const filteredColumns = computed(() => {
  return collectionColumns.value.filter((col) =>
...
const confirmMarkAsPaid = async () => {
  ...
  try {
    ...
    selectedRows.value = [];
    closePaidConfirm();
    showSuccess(t("collection.markedAsPaidSuccess"));
  } catch (error) {
    console.error("❌ Failed to mark collections as paid:", error);
    showError(
      error.message || "Failed to process collection. Please try again.",
    );
  } finally {
...
const confirmCreateInvoice = async () => {
  ...
  try {
    ...
    showSuccess(t("collection.invoiceCreatedSuccess"));
  } catch (error) {
