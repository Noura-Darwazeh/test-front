<template>
  <div class="user-page-container bg-light">
    ...
    >
      <div
        class="alert alert-danger alert-dismissible fade show shadow-lg"
        role="alert"
...
onMounted(async () => {
  try {
    await Promise.all([linesStore.fetchLines(), regionsStore.fetchRegions()]);
  } catch (error) {
...
const bulkConfirmMessage = computed(() => {
  ...
  const entityName =
    count === 1 ? t("lines.entitySingular") : t("lines.entityPlural");

  if (pendingBulkAction.value === "delete") {
    return t("common.bulkDeleteConfirm", { count, entity: entityName });
  } else if (pendingBulkAction.value === "permanentDelete") {
    return t("common.bulkPermanentDeleteConfirm", {
      count,
      entity: entityName,
    });
